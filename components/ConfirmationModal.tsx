    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden animate-fade-in-down">
        ...
        <div className="p-4 bg-slate-50 flex gap-2 border-t">
          <button onClick={onClose} className="flex-1 py-2 bg-white border border-slate-300 rounded-xl font-bold text-xs uppercase text-slate-700 transition hover:bg-slate-100">Hủy</button>
          <button onClick={onConfirm} className="flex-1 py-2 bg-red-600 text-white rounded-xl font-bold text-xs uppercase shadow-sm transition active:scale-95 hover:bg-red-700">Xác nhận</button>
        </div>
      </div>
    </div>
  );
