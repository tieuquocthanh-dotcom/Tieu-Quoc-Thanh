...
const CreateGoodsReceipt: React.FC<{ userRole: 'admin' | 'staff' | null, user: User | null }> = ({ userRole, user }) => {
  ...
  const isAdmin = userRole === 'admin';
  const [todayReceiptsSearchTerm, setTodayReceiptsSearchTerm] = useState('');

  const summaryTodayReceipts = useMemo(() => {
    const count = todayReceipts.length;
    const total = todayReceipts.reduce((sum, r) => sum + (r.total || r.finalAmount || 0), 0);
    const debt = todayReceipts.filter(r => r.paymentStatus === 'debt').reduce((sum, r) => sum + (r.total || r.finalAmount || 0), 0);
    return { count, total, debt };
  }, [todayReceipts]);

  const filteredTodayReceipts = useMemo(() => {
    if (!todayReceiptsSearchTerm.trim()) return todayReceipts;
    return todayReceipts.filter(receipt => {
      return (
        (receipt.supplierName && searchVietnameseMatch(receipt.supplierName, todayReceiptsSearchTerm)) ||
        (receipt.warehouseName && searchVietnameseMatch(receipt.warehouseName, todayReceiptsSearchTerm)) ||
        (receipt.creatorName && searchVietnameseMatch(receipt.creatorName, todayReceiptsSearchTerm)) ||
        (receipt.paymentMethodName && searchVietnameseMatch(receipt.paymentMethodName, todayReceiptsSearchTerm)) ||
        (receipt.notes && searchVietnameseMatch(receipt.notes, todayReceiptsSearchTerm)) ||
        (receipt.items && receipt.items.some(item => item.productName && searchVietnameseMatch(item.productName, todayReceiptsSearchTerm)))
      );
    });
  }, [todayReceipts, todayReceiptsSearchTerm]);
                    ...
                    <div className="p-3 bg-slate-50 border-t border-slate-200 flex-1 overflow-y-auto pb-20">
                        <div className="space-y-3">
                            <div className="bg-slate-900 p-3.5 rounded-xl border border-slate-700 shadow-md flex justify-between items-center text-white mb-2 sticky top-0 z-10">
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">Tổng đơn nhập:</span>
                                        <span className="text-xl font-black text-primary">{summaryTodayReceipts.count}</span>
                                    </div>
                                    {summaryTodayReceipts.debt > 0 && (
                                        <div className="flex items-center gap-2">
                                            <span className="text-[11px] font-bold text-rose-400 uppercase tracking-tight">Còn nợ NCC:</span>
                                            <span className="text-sm font-black text-rose-400">{formatNumber(summaryTodayReceipts.debt)} ₫</span>
                                        </div>
                                    )}
                                </div>
                                <div className="text-right">
                                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-tight block">Tiền nhập hôm nay</span>
                                    <span className="text-2xl font-black text-yellow-400">{formatNumber(summaryTodayReceipts.total)} ₫</span>
                                </div>
                            </div>
                            ...
                            <div className="relative mb-2">
                                ...
                                <input 
                                    type="text" 
                                    placeholder="Tìm đơn nhập theo tên SP, NCC, kho, người nhập..." 
                                    value={todayReceiptsSearchTerm} 
                                    onChange={e => setTodayReceiptsSearchTerm(e.target.value)} 
                                ...
                                />
                                {todayReceiptsSearchTerm && (
                                    <button
                                        type="button"
                                        onClick={() => setTodayReceiptsSearchTerm('')}
                            ...
                            {filteredTodayReceipts.length === 0 && (
                                <div className="p-4 text-center text-xs text-slate-400 font-bold bg-white rounded-xl border border-slate-200">
                                    {todayReceiptsSearchTerm.trim() 
                                        ? `Không tìm thấy đơn nhập nào khớp với "${todayReceiptsSearchTerm}"` 
                                        : 'Chưa có đơn nhập nào hôm nay'}
                                </div>
                            )}

                            {filteredTodayReceipts.map((r, receiptIdx) => {
