...
function injectRegex(file, regex) {
    let content = fs.readFileSync(file, 'utf8');
    if (!content.includes('const NumericInput:')) {
        content = content.replace(regex, numericInputStr + "\n$&");
        fs.writeFileSync(file, content);
        console.log('Injected into', file);
    }
}

injectRegex('components/SalesHistory.tsx', /const SalesHistory:/);
injectRegex('components/QuotationManagement.tsx', /export const QuotationModal:/);
injectRegex('components/ProductManagement.tsx', /export const ProductModal:/);
