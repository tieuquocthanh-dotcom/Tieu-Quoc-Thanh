...
content = content.replace(
    /<input\s*type="text" inputMode="numeric"\s*value=\{formatNumber\(payAmount\)\}\s*onChange=\{e => setPayAmount\(Math\.min\(parseInt\(e\.target\.value\.replace\(\/\\D\/g, ''\), 10\) \|\| 0, remaining\)\)\}\s*className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary font-bold text-right"\s*\/>/,
    `<NumericInput 
        value={payAmount} 
        onChange={val => setPayAmount(Math.min(val, remaining))} 
        className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary font-bold text-right"
    />`
);
